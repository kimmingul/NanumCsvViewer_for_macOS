...
- Drag-and-drop Pivot Builder with an in-window Pivot Result panel for table and chart output; Values-only, Rows+Values, Columns+Values, and full Rows+Columns+Values layouts are supported.
...
- Use `Analysis > Pivot Table` to open the Pivot Builder, then drag a field into Values. Rows and Columns are optional; the Pivot Table and Pivot Chart tabs update in the builder's large result panel.
